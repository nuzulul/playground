...
//curl "http://localhost:3000/api/users?page=1&limit=2" -H "Authorization: Bearer TEKS_TOKEN_ANDA"
//curl "http://localhost:3000/api/users?page=2&limit=2" -H "Authorization: Bearer TEKS_TOKEN_ANDA"
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    // ==========================================
    // UPDATE ROUTE: GET /api/users dengan Pagination
    // ==========================================
    if (method === 'GET' && pathname === '/api/users') {
      if (!authenticate(req, res)) return;

      // 1. Ambil query parameter dengan nilai default (Page = 1, Limit = 10)
      const page = Math.max(1, parseInt(parsedUrl.searchParams.get('page') || '1', 10));
      const limit = Math.max(1, Math.min(100, parseInt(parsedUrl.searchParams.get('limit') || '10', 10))); // Batasi maksimal 100 data
      
      // 2. Hitung Offset untuk klausa SQL
      const offset = (page - 1) * limit;

      // 3. Hitung total seluruh data untuk meta-data frontend
      const countStmt = db.prepare('SELECT COUNT(*) as total FROM users');
      const { total } = countStmt.get();
      const totalPages = Math.ceil(total / limit);

      // 4. Ambil data spesifik halaman menggunakan LIMIT & OFFSET
      const dataStmt = db.prepare('SELECT id, name, email FROM users LIMIT ? OFFSET ?');
      const users = dataStmt.all(limit, offset);

      // 5. Kembalikan data beserta struktur navigasi (Meta-data)
